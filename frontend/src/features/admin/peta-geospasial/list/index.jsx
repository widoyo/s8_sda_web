...
import { deletePetaGeospasial } from "../../../../services/peta-geospasial";
...
const ListPage = () => {
  ...
  // Table Data
  const data = petaGeospasialData.map((item) => {
    return {
      ...item,
      url: <SpatialMap zipData={item.url} width="300px" height="300px" />,
      action: (
  ...
  const handleDelete = async (id) => {
    try {
      await deletePetaGeospasial(id);
      setParams({
        ...params,
      });
      toast.success("Geoportal has been deleted");
    } catch (error) {
  ...
  return (
    <div className="flex flex-col">
      <h1 className="text-2xl font-bold">Data Geoportal</h1>
      <br />
      <div className="flex justify-between items-center mb-4">
        ...
        <Button onClick={handleAdd}>Tambah Geoportal</Button>
      </div>
