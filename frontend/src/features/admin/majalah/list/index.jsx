...
import { deleteMajalah } from "../../../../services/majalah";
...
const ListPage = () => {
  ...
  const { majalahData, setParams, params, paginationInfo } = useMajalahData();
  ...
  // Table Data
  const data = majalahData.map((item) => {
    return {
      ...item,
      action: (
  ...
  const handleDelete = async (id) => {
    try {
      await deleteMajalah(id);
      setParams({
        ...params,
      });
      toast.success("Majalah has been deleted");
    } catch (error) {
  ...
  return (
    <div className="flex flex-col">
      <h1 className="text-2xl font-bold">Data Majalah</h1>
      <br />
      <div className="flex justify-between items-center mb-4">
        ...
        <Button onClick={handleAdd}>Tambah Majalah</Button>
      </div>
